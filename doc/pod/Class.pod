...
JavaScript has no standard way to define a "class". (Even though JavaScript is prototype based OO, it is still easy to think of objects in terms of classes).

Class.js defines a very minimal set of structure and helpers without forcing you to change your coding style drastically. It's goal is to work with many styles of coding but also to give a clear way to to the basic best practices.

=head1 AN EXAMPLE CLASS
...
Here is a more complete example with full explanation:
    ...
    // - '-strict' makes sure global variables aren't accidentally defiend. It
    //   is described in detail below.
    // - '-inc' is a list of variables that are passed to the wrapper function
    //   as a clean way to defined globals.
    ...
    //   created.
    // - The '$' is the jQuery constructor, which was declared my -inc.
...
This section describes the options available to a Class Definition string.

=over

=item -strict

-strict tells Class to count the number of global variables before and after class creation. It throws an error if you add new globals (without using the special this.addGlobal() function).
...
=item -inc=value1,value2

This option defines a list of important variables that you want passed into your class wrapper as lexical variables. This is mostly sugar, but it puts all the important information about your class in the first line:
...
The -inc is a comma separated list of values. Some of these values are special:

    - this: A pointer to the current class object.
    - prototype: A pointer to the surrent class prototype object.

Anything else is a global variable.

=back

=head1 CLASS OBJECT PROPERTIES

Your class (pointed to by 'this' in your wrapper function) has a few handy properties:

=head2 this.className

The name of your class. ('Foo.Bar')

=head2 this.baseClassName

The name of your base class. ('Foo')

=head2 this.global

A pointer to the global namespace object. Usually 'window' in a browser environment.

=head2 this.addGlobal()

A function that increments the expected global count for -strict, and then returns the 'global' pointer.

=head2 this.isa(parentClassName)

This sets up inheritance manually.
